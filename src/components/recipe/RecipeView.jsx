  ...
  const renderIngredients = () => {
    if (recipe) {
      return recipe.recipe_ingredients.map((i, index) => {
        return (
          <li key={index}>
            {i.measurement_unit_quantity} {i.measurement_unit_type}{" "}
            {i.ingredient_name}
          </li>
        );
      });
  ...
  if (recipe) {
    return (
      <div className="flex-col w-full h-screen">
          ...
          <div className="w-1/2 shadow-lg rounded shadow-lg">
            <div className="flex justify-between items-baseline rounded">
              <div className="flex flex-col ml-2 mb-3">
                ...
                <p className="text-xs ml-2 gray-200">
                  {recipe ? recipe.genre : "..loading"}
                </p>
              </div>
            ...
            <div className="mt-5 flex flex-row w-full">
              <div className="flex flex-col m-auto items-start w-1/3 mt-2 bg-indigo-10">
                ...
                <ul className="list ml-2 border-b border-blue-200 divide-y divide-blue-200">
                  {renderIngredients()}
                </ul>
                <div className="text-xl mt-4 mb-3">
                  Instructions<span className="text-xs">:</span>
                </div>
                <ul className="list ml-2 border-b border-blue-200 divide-y divide-blue-200">
                  {renderInstructions()}
                </ul>
        ...
        <div className="flex flex-row justify-center align-evenly w-full h-full">
