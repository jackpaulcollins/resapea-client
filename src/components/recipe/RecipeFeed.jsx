...
const RecipeFeed = (props) => {
  ...
  async function fetchFilteredRecipes() {
    setLoading(true)
    const body = { recipe: { query_string: queryString, compatibilities: recipeCompatibilities, filter: true, page: currentPage } }
    fetch(`${API_ROOT}/api/recipes_query`, {
      headers: {'Content-Type': 'application/json'},
      method: 'post',
      credentials: 'include',
      ...
      <div className="flex flex-row justify-center w-1/2 mb-10">
        <form onSubmit={(e) => handleQuerySubmit(e)} className="w-2/3">
          <div className="relative text-gray-600 focus-within:text-gray-400">
            ...
            <input onChange={e => setQueryString(e.target.value)}type="search" name="q" className=" w-full py-2 text-sm text-blue rounded-full pl-10 focus:outline-none focus:bg-white focus:text-gray-900" placeholder="Search by name, genre, or cuisine type" autoComplete="off"/>
          </div>
        </form>
      </div>
